...
import {  useGetUsersByIdQuery, useUpdateUserMutation } from '../../slices/userApiSlice'

const UserEditScreen = () => {
    const navigate = useNavigate()
    const {id}= useParams()
    const {data:user ,isLoading:loadingUser,error} = useGetUsersByIdQuery(id)
    ...
    return (
        <div className="flex flex-col justify-center items-center">
            ...
            <form className="w-full max-w-sm" onSubmit={handleUpdateUser}>
                ...
                <button
                    type="button"
                    className="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600" onClick={handleUpdateUser}
                >
                    Update
                </button>
                {isLoading && <Spinner/>}
